...
| Spring Boot Build-Image (Buildpacks) | Google Jib Maven Plugin |
| Logback | Log4j2 |
...
Перед запуском микросервисов необходимо развернуть инфраструктуру и настроить Keycloak для аутентификации.

<details>
<summary>Сборка микросервисов</summary>
В терминале, находясь в корневом каталоге проекта, выполнить сборку JAR-файлов и Docker-образов:
...
|:-------|:----:|:-----------|
| twitter-to-kafka-service | 5005 | Получение/генерация потока твитов |
| config-server | 5006 | Общий сервер конфигураций |

Список запущенной инфраструктуры:
| Сервис | Порт | Назначение |
|:-------|:----:|:-----------|
